...
$page_title = 'Edit Keyboard';
require_once('includes/load.php');
page_require_level(3);

$keyboard = find_by_id('keyboard', (int)$_GET['id']);
if (!$keyboard) {
    $session->msg("d", "Missing category id.");
    redirect('bar3.php');
}

function find_keyboard_by_id($id) {
  global $db;
  $sql = "SELECT * FROM keyboard WHERE id = '{$id}' LIMIT 1";
  $result = $db->query($sql);
...
if(isset($_GET['id'])){
    $keyboard_id = (int)$_GET['id'];
    $keyboard_file = find_keyboard_by_id($keyboard_id);
    if(!$keyboard_file){
        $session->msg('d', 'Keyboard file not found.');
        redirect('bar3.php');
    }
}
...
$exclude_id = isset($keyboard_id) ? $keyboard_id : null;

$check_duplicate = "SELECT * FROM keyboard WHERE file_name = '{$file_name}'";
...
if ($result && $db->num_rows($result) > 0) {
    ...
    redirect('bar3.php?error=true&message=' . urlencode($duplicate_error), false);
}
  ...
  if (move_uploaded_file($tmp_name, $file_path)) {
      ...
      $sql = "UPDATE keyboard SET file_name='{$file_name}' WHERE id='{$keyboard['id']}'";
      $result = $db->query($sql);

      // Check if the query was successful
      if ($result) {
          redirect('bar3.php?success=true&Photos=true', false);
      } else {
          $session->msg('d', 'Failed to update keyboard.');
      }
  } else {
      ...
      <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
        <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading">
          <div class="panel-heading clearfix">
           ...
           <span>Editing <strong><?php echo remove_junk(ucfirst($keyboard['file_name']));?></strong></span>
          </div>
        </div>
        <div class="panel-body">
          <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <img class="img-thumbnail" src="uploads/products/<?php echo $keyboard_file['file_name'];?>" alt="">
            </div>
            <div class="col-md-8">
              <form class="form" action="edit_bar3?id=<?php echo $keyboard['id']; ?>" method="POST" enctype="multipart/form-data">
              ...
              <center>
                ...
                <a style="border-radius: 50% 10% 50% 10% / 10% 50% 10% 50%; box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" href="bar3.php" class="btn btn-danger">Cancel</a>
              </center>
             </form>
            </div>
          </div>
