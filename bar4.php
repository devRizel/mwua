...
?>
<?php $mouse_files = find_all('mouse');?>
<?php
if (isset($_POST['submit'])) {
  $photo = new mouse();
  $photo->upload($_FILES['file_upload']);
  
  if ($photo->process_mouse()) {
      $session->msg('s', 'Barcode Photo has been uploaded.');
      redirect('bar4.php?success=true'); // Redirect with success parameter
  } else {
...
<?php endif; ?>
<!--END-->
<center><h1>Add Barcode|Mouse</h1></center>
     <div class="row">
        ...
        <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
          <div  style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading clearfix">
            ...
            <div class="pull-right">
              <form class="form-inline" action="bar4" method="POST" enctype="multipart/form-data">
          ...
          <div class="panel-body">
            <table class="table">
              <thead>
                ...
                <tbody>
                <?php foreach ($mouse_files as $mouse_file): ?>
                <tr class="list-inline">
                 <td class="text-center"><?php echo count_id();?></td>
                  <td class="text-center">
                      <img src="uploads/products/<?php echo $mouse_file['file_name'];?>" class="img-thumbnail" />
                  </td>
                <td class="text-center">
                  <?php echo $mouse_file['file_name'];?>
                </td>
                <td class="text-center">
                  <?php echo $mouse_file['file_type'];?>
                </td>
                <td class="text-center">
                <a href="edit_bar4.php?id=<?php echo (int) $mouse_file['id'];?>" class="btn btn-info btn-xs"  title="Edit">
                    <span class="glyphicon glyphicon-edit"></span>
                  </a>
                  <a href="delete_bar4.php?id=<?php echo (int) $mouse_file['id'];?>" class="btn btn-danger btn-xs"  title="Delete">
