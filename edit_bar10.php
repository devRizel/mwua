...
$page_title = 'Edit chord2';
require_once('includes/load.php');
page_require_level(3);

$chord2 = find_by_id('chord2', (int)$_GET['id']);
if (!$chord2) {
    $session->msg("d", "Missing category id.");
    redirect('bar10.php');
}

function find_chord2_by_id($id) {
  global $db;
  $sql = "SELECT * FROM chord2 WHERE id = '{$id}' LIMIT 1";
  $result = $db->query($sql);
...
if(isset($_GET['id'])){
    $chord2_id = (int)$_GET['id'];
    $chord2_file = find_chord2_by_id($chord2_id);
    if(!$chord2_file){
        $session->msg('d', 'chord2 file not found.');
        redirect('bar10.php');
    }
}
...
$exclude_id = isset($chord2_id) ? $chord2_id : null;

$check_duplicate = "SELECT * FROM chord2 WHERE file_name = '{$file_name}'";
...
if ($result && $db->num_rows($result) > 0) {
    ...
    redirect('bar10.php?error=true&message=' . urlencode($duplicate_error), false);
}
  ...
  if (move_uploaded_file($tmp_name, $file_path)) {
      ...
      $sql = "UPDATE chord2 SET file_name='{$file_name}' WHERE id='{$chord2['id']}'";
      $result = $db->query($sql);

      // Check if the query was successful
      if ($result) {
          redirect('bar10.php?success=true&Photos=true', false);
      } else {
          $session->msg('d', 'Failed to update chord2.');
      }
  } else {
      ...
      <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
        <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading">
          <div class="panel-heading clearfix">
           ...
           <span>Editing <strong><?php echo remove_junk(ucfirst($chord2['file_name']));?></strong></span>
          </div>
        </div>
        <div class="panel-body">
          <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <img class="img-thumbnail" src="uploads/products/<?php echo $chord2_file['file_name'];?>" alt="">
            </div>
            <div class="col-md-8">
              <form class="form" action="edit_bar10?id=<?php echo $chord2['id']; ?>" method="POST" enctype="multipart/form-data">
              ...
              <center>
                ...
                <a style="border-radius: 50% 10% 50% 10% / 10% 50% 10% 50%; box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" href="bar10.php" class="btn btn-danger">Cancel</a>
              </center>
             </form>
            </div>
          </div>
