...
if(isset($_GET['id'])){
    ...
    $media_file = find_media_by_id($media_id);
    if(!$media_file){
        $session->msg('d', 'Media file not found.');
        redirect('media.php');
    }
}
...
$exclude_id = isset($media_id) ? $media_id : null;

$check_duplicate = "SELECT * FROM media WHERE file_name = '{$file_name}'";
...
if ($result && $db->num_rows($result) > 0) {
    ...
    redirect('media.php?error=true&message=' . urlencode($duplicate_error), false);
}
  ...
  if (move_uploaded_file($tmp_name, $file_path)) {
      ...
      $sql = "UPDATE media SET file_name='{$file_name}' WHERE id='{$media['id']}'";
      $result = $db->query($sql);

      // Check if the query was successful
      if ($result) {
          redirect('media.php?success=true&Photos=true', false);
      } else {
          $session->msg('d', 'Failed to update media.');
      }
  } else {
      ...
      <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
        <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading">
          <div class="panel-heading clearfix">
           ...
           <span>Editing <strong><?php echo remove_junk(ucfirst($media['file_name']));?></strong></span>
          </div>
        </div>
        <div class="panel-body">
          <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <img class="img-thumbnail" src="uploads/products/<?php echo $media_file['file_name'];?>" alt="">
            </div>
            <div class="col-md-8">
              <form class="form" action="edit_media?id=<?php echo $media['id']; ?>" method="POST" enctype="multipart/form-data">
              <div class="form-group col-md-offset-5">
                <input  style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.4);" type="file" name="file_name" id="file_name" class="btn btn-file"/>
              </div>
              <div class="form-group col-md-offset-6">
              <center>
                <button style="border-radius: 50% 10% 50% 10% / 10% 50% 10% 50%; box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" type="submit" name="edit_media" class="btn btn-primary">Update Images</button>
                <a style="border-radius: 50% 10% 50% 10% / 10% 50% 10% 50%; box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" href="media.php" class="btn btn-danger">Cancel</a>
              </center>
             </form>
            </div>
          </div>
