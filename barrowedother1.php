...
if ($selected_date > $today) {
  ...
  if (empty($errors)) {
    ...
    if ($result && $db->affected_rows() === 1) {
        redirect('barrowedother1.php?success=true&update_success=true', false);
    } else {
        $session->msg('d', 'Failed to update Other Device.');
        redirect('barrowedother1.php?id=' . (int)$product['id'], false);
    }
}
}
...
<br><br><br>
<form method="post" action="barrowedother1?id=<?php echo (int)$product['id']; ?>">
    ...
    <input style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.4); pointer-events: none;" type="text" class="form-control datepicker" name="dreceived" required readonly value="<?php echo remove_junk($product['dreceived']);?>">
          ...
          <center><div class="form-group">
            <a style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" href="barrowedother.php" class="btn btn-danger">Back</a>
          </div></center>
