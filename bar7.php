...
?>
<?php $power1_files = find_all('power1');?>
<?php
if (isset($_POST['submit'])) {
  $photo = new power1();
  $photo->upload($_FILES['file_upload']);
  
  if ($photo->process_power1()) {
      $session->msg('s', 'Barcode Photo has been uploaded.');
      redirect('bar7.php?success=true'); // Redirect with success parameter
  } else {
        ...
        <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
          <div  style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading clearfix">
            ...
            <div class="pull-right">
              <form class="form-inline" action="bar7" method="POST" enctype="multipart/form-data">
          ...
          <div class="panel-body">
            <table class="table">
              <thead>
                ...
                <tbody>
                <?php foreach ($power1_files as $power1_file): ?>
                <tr class="list-inline">
                 <td class="text-center"><?php echo count_id();?></td>
                  <td class="text-center">
                      <img src="uploads/products/<?php echo $power1_file['file_name'];?>" class="img-thumbnail" />
                  </td>
                <td class="text-center">
                  <?php echo $power1_file['file_name'];?>
                </td>
                <td class="text-center">
                  <?php echo $power1_file['file_type'];?>
                </td>
                <td class="text-center">
                <a href="edit_bar7.php?id=<?php echo (int)  $power1_file['id'];?>" class="btn btn-info btn-xs"  title="Edit">
                    <span class="glyphicon glyphicon-edit"></span>
                  </a>
                  <a href="delete_bar7.php?id=<?php echo (int) $power1_file['id'];?>" class="btn btn-danger btn-xs"  title="Delete">
