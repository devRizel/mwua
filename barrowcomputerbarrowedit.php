...
if (empty($_POST['datebarrowed'])) {
  ...
  if ($selected_date > $today) {
    ...
    if (empty($errors)) {
      ...
      $query = "UPDATE products SET barrow = '{$p_barrow}', datebarrowed = '{$p_datebarrowed}',datereturn = '{$p_datereturn}' WHERE id = '{$product['id']}'";
      $result = $db->query($query);

        if ($result && $db->affected_rows() === 1) {
            redirect('barrowcomputer.php?success=true&update_success=true', false);
        } else {
            $session->msg('d', 'Failed to update Computer.');
            redirect('barrowcomputerbarrowedit.php?id=' . (int)$product['id'], false);
        }
    }
}
    ...
    <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
      <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading">
        <strong>
          ...
          <span>Computer Units Barrow</span>
        </strong>
      </div>
      <div class="panel-body">
        <form method="post" class="clearfix" action="barrowcomputerbarrowedit?id=<?php echo (int)$product['id'] ?>" >
          ...
          <center><div class="form-group">
            ...
            <a style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" href="barrowcomputer.php" class="btn btn-danger">Cancel</a>
          </div></center>
        </form>
      </div>
    </div>
