...
if (isset($_POST['edit_cat'])) {
  ...
  if (empty($errors)) {
      ...
      $check_duplicate = $db->query("SELECT * FROM room WHERE name = '{$cat_name}' AND id != '{$categorie['id']}'");

      if ($check_duplicate->num_rows > 0) {
          // Duplicate found, set an error message
          $duplicate_error = "Please avoid using Duplicate Room!";
          ...
          $sql = "UPDATE room SET name='{$cat_name}' WHERE id='{$categorie['id']}'";
          $result = $db->query($sql);
      ...
      redirect('edit_categorie2.php?id=' . (int)$categorie['id'], false);
  }
}
?>
   ...
   <div class="col-md-5">
     <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
       ...
       <div class="panel-body">
         <form method="post" action="edit_categorie2?id=<?php echo (int)$categorie['id'];?>">
