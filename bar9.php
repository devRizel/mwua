...
?>
<?php $chord1_files = find_all('chord1');?>
<?php
if (isset($_POST['submit'])) {
  $photo = new chord1();
  $photo->upload($_FILES['file_upload']);
  
  if ($photo->process_chord1()) {
      $session->msg('s', 'Barcode Photo has been uploaded.');
      redirect('bar9.php?success=true'); // Redirect with success parameter
  } else {
...
<?php endif; ?>
<!--END-->
<center><h1>Add Barcode|POWER CHORD1</h1></center>
     <div class="row">
        ...
        <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
          <div  style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading clearfix">
            ...
            <div class="pull-right">
              <form class="form-inline" action="bar9" method="POST" enctype="multipart/form-data">
          ...
          <div class="panel-body">
            <table class="table">
              <thead>
                ...
                <tbody>
                <?php foreach ($chord1_files as $chord1_file): ?>
                <tr class="list-inline">
                 <td class="text-center"><?php echo count_id();?></td>
                  <td class="text-center">
                      <img src="uploads/products/<?php echo $chord1_file['file_name'];?>" class="img-thumbnail" />
                  </td>
                <td class="text-center">
                  <?php echo $chord1_file['file_name'];?>
                </td>
                <td class="text-center">
                  <?php echo $chord1_file['file_type'];?>
                </td>
                <td class="text-center">
                <a href="edit_bar9.php?id=<?php echo (int)  $chord1_file['id'];?>" class="btn btn-info btn-xs"  title="Edit">
                    <span class="glyphicon glyphicon-edit"></span>
                  </a>
                  <a href="delete_bar9.php?id=<?php echo (int) $chord1_file['id'];?>" class="btn btn-danger btn-xs"  title="Delete">
