...
?>
<?php $ram_files = find_all('ram');?>
<?php
if (isset($_POST['submit'])) {
  $photo = new ram();
  $photo->upload($_FILES['file_upload']);
  
  if ($photo->process_ram()) {
      $session->msg('s', 'Barcode Photo has been uploaded.');
      redirect('bar13.php?success=true'); // Redirect with success parameter
  } else {
...
<?php endif; ?>
<!--END-->
<center><h1>Add Barcode|RAM</h1></center>
     <div class="row">
        ...
        <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
          <div  style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading clearfix">
            ...
            <div class="pull-right">
              <form class="form-inline" action="bar13" method="POST" enctype="multipart/form-data">
          ...
          <div class="panel-body">
            <table class="table">
              <thead>
                ...
                <tbody>
                <?php foreach ($ram_files as $ram_file): ?>
                <tr class="list-inline">
                 <td class="text-center"><?php echo count_id();?></td>
                  <td class="text-center">
                      <img src="uploads/products/<?php echo $ram_file['file_name'];?>" class="img-thumbnail" />
                  </td>
                <td class="text-center">
                  <?php echo $ram_file['file_name'];?>
                </td>
                <td class="text-center">
                  <?php echo $ram_file['file_type'];?>
                </td>
                <td class="text-center">
                <a href="edit_bar13.php?id=<?php echo (int) $ram_file['id'];?>" class="btn btn-info btn-xs"  title="Edit">
                    <span class="glyphicon glyphicon-edit"></span>
                  </a>
                  <a href="delete_bar13.php?id=<?php echo (int) $ram_file['id'];?>" class="btn btn-danger btn-xs"  title="Delete">
