...
$page_title = 'Edit mother';
require_once('includes/load.php');
page_require_level(3);

$mother = find_by_id('mother', (int)$_GET['id']);
if (!$mother) {
    $session->msg("d", "Missing category id.");
    redirect('bar11.php');
}

function find_mother_by_id($id) {
  global $db;
  $sql = "SELECT * FROM mother WHERE id = '{$id}' LIMIT 1";
  $result = $db->query($sql);
...
if(isset($_GET['id'])){
    $mother_id = (int)$_GET['id'];
    $mother_file = find_mother_by_id($mother_id);
    if(!$mother_file){
        $session->msg('d', 'mother file not found.');
        redirect('bar11.php');
    }
}
...
$exclude_id = isset($mother_id) ? $mother_id : null;

$check_duplicate = "SELECT * FROM mother WHERE file_name = '{$file_name}'";
...
if ($result && $db->num_rows($result) > 0) {
    ...
    redirect('bar11.php?error=true&message=' . urlencode($duplicate_error), false);
}
  ...
  if (move_uploaded_file($tmp_name, $file_path)) {
      ...
      $sql = "UPDATE mother SET file_name='{$file_name}' WHERE id='{$mother['id']}'";
      $result = $db->query($sql);

      // Check if the query was successful
      if ($result) {
          redirect('bar11.php?success=true&Photos=true', false);
      } else {
          $session->msg('d', 'Failed to update mother.');
      }
  } else {
      ...
      <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
        <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading">
          <div class="panel-heading clearfix">
           ...
           <span>Editing <strong><?php echo remove_junk(ucfirst($mother['file_name']));?></strong></span>
          </div>
        </div>
        <div class="panel-body">
          <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <img class="img-thumbnail" src="uploads/products/<?php echo $mother_file['file_name'];?>" alt="">
            </div>
            <div class="col-md-8">
              <form class="form" action="edit_bar11?id=<?php echo $mother['id']; ?>" method="POST" enctype="multipart/form-data">
              ...
              <center>
                ...
                <a style="border-radius: 50% 10% 50% 10% / 10% 50% 10% 50%; box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" href="bar11.php" class="btn btn-danger">Cancel</a>
              </center>
             </form>
            </div>
          </div>
