...
// Filter out only the "Server Room"
$filtered_rooms = array_filter($all_room, function($room) {
  return $room['name'] == 'Faculty';
});
...
$query .= "('{$p_name}', '{$p_cat}', '{$p_recievedby}','{$p_donate}' ,'{$p_monitor}', '{$p_keyboard}', '{$p_mouse}', '{$p_v1}', '{$p_p1}', '{$p_p2}', '{$p_power1}', '{$p_system}', '{$p_mother}', '{$p_cpu}', '{$p_ram}', '{$p_power2}', '{$p_video}', '{$p_h}', '{$media_id}', '{$date}', '{$date_received}')";
       ...
       if ($db->query($query)) {
           ...
           header("Location: add_product11.php?success=true");
           exit();
       } else {
           // Failed to add computer
           $session->msg('d', 'Failed to add computer!');
           header("Location: add_product11.php");
           exit();
       }
   } else {
  ...
  <div class="col-md-offset-2 col-md-8 ">
    <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
      ...
      <div class="panel-body">
        <div class="col-md-12">
        <form method="post" action="add_product11" class="clearfix">
    ...
    <div class="form-group">
        <div class="row">
            ...
            <center><div class="form-group ">
              ...
              <a style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" href="product1.php" class="btn btn-danger" class="">Cancel</a>
            </div></center>
          </form>
        </div>
      </div>
...
if (successParam === 'true') {
    swal("", "Computer added successfully", "success")
        .then((value) => {
            ...
            window.location.href = 'add_product11.php';
        });
}
</script>
