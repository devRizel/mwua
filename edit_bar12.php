...
$cpu = find_by_id('cpu', (int)$_GET['id']);
if (!$cpu) {
    $session->msg("d", "Missing category id.");
    redirect('bar12.php');
}

function find_cpu_by_id($id) {
  global $db;
  $sql = "SELECT * FROM cpu WHERE id = '{$id}' LIMIT 1";
  $result = $db->query($sql);
...
if(isset($_GET['id'])){
    $cpu_id = (int)$_GET['id'];
    $cpu_file = find_cpu_by_id($cpu_id);
    if(!$cpu_file){
        $session->msg('d', 'cpu file not found.');
        redirect('bar12.php');
    }
}
...
$exclude_id = isset($cpu_id) ? $cpu_id : null;

$check_duplicate = "SELECT * FROM cpu WHERE file_name = '{$file_name}'";
...
if ($result && $db->num_rows($result) > 0) {
    ...
    redirect('bar12.php?error=true&message=' . urlencode($duplicate_error), false);
}
  ...
  if (move_uploaded_file($tmp_name, $file_path)) {
      ...
      $sql = "UPDATE cpu SET file_name='{$file_name}' WHERE id='{$cpu['id']}'";
      $result = $db->query($sql);

      // Check if the query was successful
      if ($result) {
          redirect('bar12.php?success=true&Photos=true', false);
      } else {
          $session->msg('d', 'Failed to update cpu.');
      }
  } else {
      ...
      <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
        <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading">
          <div class="panel-heading clearfix">
           ...
           <span>Editing <strong><?php echo remove_junk(ucfirst($cpu['file_name']));?></strong></span>
          </div>
        </div>
        <div class="panel-body">
          <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <img class="img-thumbnail" src="uploads/products/<?php echo $cpu_file['file_name'];?>" alt="">
            </div>
            <div class="col-md-8">
              <form class="form" action="edit_bar12?id=<?php echo $cpu['id']; ?>" method="POST" enctype="multipart/form-data">
              ...
              <center>
                ...
                <a style="border-radius: 50% 10% 50% 10% / 10% 50% 10% 50%; box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" href="bar12.php" class="btn btn-danger">Cancel</a>
              </center>
             </form>
            </div>
          </div>
