...
$page_title = 'Edit hddssdgb';
require_once('includes/load.php');
page_require_level(3);

$hddssdgb = find_by_id('hddssdgb', (int)$_GET['id']);
if (!$hddssdgb) {
    $session->msg("d", "Missing category id.");
    redirect('bar15.php');
}

function find_hddssdgb_by_id($id) {
  global $db;
  $sql = "SELECT * FROM hddssdgb WHERE id = '{$id}' LIMIT 1";
  $result = $db->query($sql);
...
if(isset($_GET['id'])){
    $hddssdgb_id = (int)$_GET['id'];
    $hddssdgb_file = find_hddssdgb_by_id($hddssdgb_id);
    if(!$hddssdgb_file){
        $session->msg('d', 'hddssdgb file not found.');
        redirect('bar15.php');
    }
}
...
$exclude_id = isset($hddssdgb_id) ? $hddssdgb_id : null;

$check_duplicate = "SELECT * FROM hddssdgb WHERE file_name = '{$file_name}'";
...
if ($result && $db->num_rows($result) > 0) {
    ...
    redirect('bar15.php?error=true&message=' . urlencode($duplicate_error), false);
}
  ...
  if (move_uploaded_file($tmp_name, $file_path)) {
      ...
      $sql = "UPDATE hddssdgb SET file_name='{$file_name}' WHERE id='{$hddssdgb['id']}'";
      $result = $db->query($sql);

      // Check if the query was successful
      if ($result) {
          redirect('bar15.php?success=true&Photos=true', false);
      } else {
          $session->msg('d', 'Failed to update hddssdgb.');
      }
  } else {
      ...
      <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel panel-default">
        <div style="box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" class="panel-heading">
          <div class="panel-heading clearfix">
           ...
           <span>Editing <strong><?php echo remove_junk(ucfirst($hddssdgb['file_name']));?></strong></span>
          </div>
        </div>
        <div class="panel-body">
          <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <img class="img-thumbnail" src="uploads/products/<?php echo $hddssdgb_file['file_name'];?>" alt="">
            </div>
            <div class="col-md-8">
              <form class="form" action="edit_bar15?id=<?php echo $hddssdgb['id']; ?>" method="POST" enctype="multipart/form-data">
              ...
              <center>
                ...
                <a style="border-radius: 50% 10% 50% 10% / 10% 50% 10% 50%; box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);" href="bar15.php" class="btn btn-danger">Cancel</a>
              </center>
             </form>
            </div>
          </div>
